...
export async function plan(origin, dest, bays, opts = {}) {
  ...
  // Caller can restrict transit modes and turn the bike off (Extras "avoid").
  ...
  if (opts.stations) {
    ...
    stations = allowBike ? opts.stations.filter((s) => haversine(origin, s) <= 4500) : [];
  } else {
    [baselineJ, stations] = await Promise.all([
      tfl.journey(origin, dest, modes, opts).catch(() => []),
      allowBike ? tfl.nearbyStations(origin, 3000).catch(() => []) : Promise.resolve([]),
    ]);
  }
