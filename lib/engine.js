...
// TfL cycle routing assumes a normal pedal bike. Lime/Forest e-bikes are
// quite a bit faster, so scale TfL's cycle time down to reflect that.
const EBIKE_FACTOR = 0.75;
...
export async function plan(origin, dest, bays, opts = {}) {
  ...
  if (opts.stations) {
    baselineJ = await tfl.journey(origin, dest, TRANSIT_MODES, opts).catch(() => []);
    stations = opts.stations.filter((s) => haversine(origin, s) <= 4500);
  } else {
    [baselineJ, stations] = await Promise.all([
      tfl.journey(origin, dest, TRANSIT_MODES, opts).catch(() => []),
      tfl.nearbyStations(origin, 3000).catch(() => []),
    ]);
  }
