...
// Lime e-bike pricing (the app's dockless bike): ~£1 unlock + ~£0.29/min, OR a
// 30-min pass for £3.99 — cheaper for anything over ~14 min, and a single pass
// covers a there-and-back trip whose total riding stays within 30 min.
export const BIKE_RATES = {
  lime: { unlock: 1.0, perMin: 0.29, passMins: 30, passPrice: 3.99, label: "Lime" },
};

// Cheapest way to ride `minutes` on Lime — pay-as-you-go vs a time pass — with
// the full workings so the UI can explain it (and suggest a pass / return trip).
export function bikePricing(minutes) {
  const r = BIKE_RATES.lime;
  ...
  const passesNeeded = Math.max(1, Math.ceil(m / r.passMins));
  const passUnitPence = Math.round(r.passPrice * 100);
  const passPence = passesNeeded * passUnitPence;
  const usePass = passPence <= paygPence;
  return {
    op: "Lime",
    min: m,
    ...
    returnPassCovers: 2 * m <= r.passMins,
  };
}
