...
export async function suggest(q) {
  ...
  try {
    ...
    // Surface stations first — they're usually what people are routing to.
    const isStation = (p = {}) =>
      p.osm_key === "railway" ||
      p.osm_key === "public_transport" ||
      /station|halt/i.test(p.osm_value || "") ||
      /\bstation\b/i.test(p.name || "");
    return feats
      .map((f) => {
        const [lon, lat] = f.geometry.coordinates;
        return { lat, lon, name: photonLabel(f, q), _stn: isStation(f.properties) };
      })
      .sort((a, b) => (b._stn === true) - (a._stn === true))
      .map(({ _stn, ...s }) => s);
  } catch {
    return [];
  }
